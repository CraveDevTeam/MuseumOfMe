...
namespace MOMmain
{
    public class ServerScript : MonoBehaviour
    {
        ...
        public IEnumerator SetAppData(AppData AppData, Action<bool> Callback)
        {
            ...
            if (AppData != null)
            {
                yield return StartCoroutine(IsAppDataExist(AppData.DIRID, (InnerCallback) =>
                {
                    IsExist = InnerCallback;
                }));

                if (IsExist)
                {
                    SQL = "UPDATE `appdata` SET `AppName`='" + AppData.AppName + "',`AppValues`='" + AppData.AppValues +
                        "',`LastEdit`='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + 
                        "' WHERE GUID = '" + AppData.DIRID + "' ";
                }
                else
                {
                    AppData.DIRID = AppData.DIRID == null ? Guid.NewGuid().ToString() : AppData.DIRID;
                    SQL = "INSERT INTO 'appdata'('DIRID', 'QRID', 'NickName', 'CharValue', 'DateTimeUpdated') " +
                        "VALUES ('" + AppData.DIRID + "','" + AppData.QRID + "','" + AppData.NickName + 
                        "','" + DateTime.Now.ToString("yyyyMMdd") + "','" + DateTime.Now.ToString("hhmmss") + "')";
                }
                Debug.Log(SQL);
                yield return StartCoroutine(Server.Instance.SendRequest(SQL, (ServerCallback) =>
                {
                    try
                    {
                        Successful = !ServerCallback.Contains("Error");
                    }
                    catch
                    {
                        Message("Error: " + ServerCallback);
                    }
                }, false));
            }
        ...
        public IEnumerator IsAppDataExist(string GUID, Action<bool> Callback)
        {
            bool IsExist = false;
            string SQL = "Select GUID from appdata where GUID = '" + GUID + "'";

            yield return StartCoroutine(Server.Instance.SendRequest(SQL, (ServerCallback) =>
            {
                try
                {
                    IsExist = Json.FromJson<AppData>(ServerCallback).Length > 0 ? true : false;
                }
                catch
                {
                    Message("Error: " + ServerCallback);
